...
6. 实验编号、处理编号、样品编号等结构化字段必须保留图片中直接可见的原始字符和符号。不得把 N₁ 识别为 1，不得删除或改写 `/`、`-`、`+`；下标数字应尽量按图片中的形式保留。字符不确定时使用 `[模糊字符]` 或 `?` 标记，不得通过猜测删除编号组成部分。
7. 重复实验的每一行仍然必须独立保存完整处理编号和测量值，不求平均、不合并。
8. 如果图片能够明确判断重复关系，同组行填写相同 replicate_group，并按图片顺序填写 replicate_index；不能明确判断时填写 null。
9. replicate_group 和 replicate_index 只是辅助信息，不得改变处理编号或测量值。
10. warnings 只能使用简短提示，例如“存在模糊字符”或“可能存在漏行”；禁止逐项解释和长篇分析。
