...
class ValidationPromptTests(unittest.TestCase):
    ...
    def test_prompt_is_compact_and_read_only(self):
        ...
        self.assertIn("column_name 必须逐字使用输入 columns 中的 internal_name", prompt)
        self.assertIn("row_index 必须直接使用输入 observed_rows 中从 1 开始", prompt)
        self.assertIn('location_status 必须为 unknown', prompt)
        self.assertIn('"location_status": "resolved"', prompt)
